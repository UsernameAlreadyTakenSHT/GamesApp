...
import io.github.usernamealreadytakensht.games.game.draughts.DraughtsState
import io.github.usernamealreadytakensht.games.game.draughts.DraughtsViewModel
...
sealed interface DraughtsLaunch {
    data class NewGame(val config: DraughtsConfig, val id: Int) : DraughtsLaunch
    data object Resume : DraughtsLaunch
}
...
fun DraughtsScreen(
    ...
    onBack: () -> Unit,
    onNewGame: () -> Unit,
    vm: DraughtsViewModel = viewModel(),
) {
        ...
        Column(
            ...
            Row(
                ...
                Row(verticalAlignment = Alignment.CenterVertically) {
                    ...
                    Text("Draughts", style = MaterialTheme.typography.titleLarge)
                }
...
@Composable
private fun Board(state: DraughtsState, onTap: (Int) -> Unit) {
    val lastSquares = state.lastMove?.let { setOf(it.from) + it.path } ?: emptySet()
    Column(modifier = Modifier.fillMaxWidth().aspectRatio(1f)) {
        for (r in 0 until 10) {
            Row(modifier = Modifier.weight(1f).fillMaxWidth()) {
                for (c in 0 until 10) {
                    val row = if (state.flipped) 9 - r else r
                    val col = if (state.flipped) 9 - c else c
                    val sq = Draughts.squareAt(row, col)
                    val playable = sq != 0
                    Box(
                        modifier = Modifier
                            .weight(1f)
                        ...
                        if (playable) {
                            Text("$sq", fontSize = 8.sp, color = LightSquare,
                            ...
                            if (piece != Draughts.EMPTY) {
                                val res = when (piece) {
                                    Draughts.WHITE_MAN -> R.drawable.stone_l1
                                    Draughts.WHITE_KING -> R.drawable.stone_l2
                                    Draughts.BLACK_MAN -> R.drawable.stone_d1
...
@Composable
private fun GameOverBanner(state: DraughtsState, onRematch: () -> Unit, onNewGame: () -> Unit) {
